...
def get_questions():
    ...
    for question in questions:
        try:
            ...
            member_id = question.get("memberId", "unknown")
            if isinstance(member_id, ObjectId):
                member_id = str(member_id)
            
            date_created = question.get("dateCreated")
            if isinstance(date_created, datetime):
                date_str = date_created.isoformat()
            else:
                # Fallback to now if missing/bad
                date_str = datetime.utcnow().isoformat()
                print(f"Warning: Invalid dateCreated for question {question.get('_id', 'unknown')}: {date_created}")
            ...
            response.append({
                ...
                "communityId": question.get("communityId", 0),
                "memberId": member_id,
                "score": question.get("score", 0),
                "views": question.get("views", 0),
                "answers": answers_count
            })
            print(f"Processed question: {question.get('title', 'Untitled')}")  # Debug per doc
        except Exception as e:
            print(f"Skipping bad question {question.get('_id', 'unknown')}: {str(e)}")  # Log bad doc
            continue  # Skip, don't crash whole route
    
    print(f"Returning {len(response)} valid questions")  # Overall debug
    return jsonify(response), 200
...
@app.route('/recommended-questions', methods=['GET'])  # Changed to hyphen
@login_required
def get_recommended_questions():
    ...
    try:
        ...
        if not community_ids:
            print("No communities for user, returning empty")
            return jsonify([]), 200
        ...
        for question in questions:
            try:
                ...
                member = mongo.db.users.find_one({"_id": ObjectId(question.get("memberId", ""))})
                ...
                print(f"Skipping bad recommended question {question.get('_id', 'unknown')}: {str(e)}")
                continue
        
        print(f"Returning {len(response)} recommended questions")
        return jsonify(response), 200
    except Exception as e:
...
def increment_question_views(question_id):
    ...
    try:
        ...
        try:
            ...
            print(f"Invalid question_id: {question_id}, error: {oid_err}")
        ...
        print(f"Error in increment_question_views: {str(e)}")
